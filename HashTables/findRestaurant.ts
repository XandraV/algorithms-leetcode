// Given two arrays of strings list1 and list2, find the common strings with the least index sum.
// A common string is a string that appeared in both list1 and list2.
// A common string with the least index sum is a common string such that if it appeared
//  at list1[i] and list2[j] then i + j should be the minimum value among all the other common strings.
// Return all the common strings with the least index sum. Return the answer in any order.
function findRestaurant(list1: string[], list2: string[]): string[] {
  let minSum: number = Infinity;
  let resWords: string[] = [];

  const idxMap: Map<string, number> = new Map();
  const smallerArray = list1.length > list2.length ? list2 : list1;
  const largerArray = list1.length > list2.length ? list1 : list2;

  for (let i = 0; i < smallerArray.length; i++) {
    idxMap.set(smallerArray[i], i);
  }

  console.log(idxMap);
  for (let j = 0; j < largerArray.length; j++) {
    let curr = largerArray[j];
    const idxInOtherArray = idxMap.get(curr); // i
    if (idxInOtherArray !== undefined) {
      // instead of idxMap.has(curr);
      let currSum = idxInOtherArray + j;
      if (currSum < minSum) {
        resWords = [curr];
        minSum = currSum;
      } else if (currSum === minSum) {
        resWords.push(curr);
      }
    }
  }

  return resWords;
}

const list1 = ["Shogun", "Tapioca Express", "Burger King", "KFC"],
  list2 = ["KFC", "Shogun", "Burger King"];
// ["Shogun"]
console.log(findRestaurant(list1, list2));
